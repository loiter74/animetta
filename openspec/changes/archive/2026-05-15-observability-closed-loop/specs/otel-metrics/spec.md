...
The system SHALL initialize an OpenTelemetry MeterProvider alongside the existing TracerProvider during `init_tracing()`, exporting metrics to the OTel Collector via OTLP.

#### Scenario: MeterProvider created at startup
- **WHEN** the backend starts and `init_tracing()` is called
- **THEN** a MeterProvider SHALL be created with the same `service.name` resource attribute as the TracerProvider
- **THEN** a PeriodicExportingMetricReader SHALL be configured with an OTLP gRPC metric exporter pointing to `http://localhost:4317`
