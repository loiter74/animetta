...
class PeriodicLearner:
    ...
    def __init__(
        ...
        self._mbti_enabled = config.get("mbti_analysis_enabled", True)

        self._mbti_analyzer = MBTIAnalyzer(
            llm_client=llm_client,
            config=config.get("mbti_analysis", {}),
        ) if self._mbti_enabled else None
    ...
    async def analyze_mbti(self) -> None:
        """Scheduled task: analyze MBTI dimensions from recent conversations.

        Runs MBTIAnalyzer on recent summaries, applies guardrails,
        and saves updated profile via MBTIStore.
        """
        if not self._mbti_analyzer:
            logger.debug("[PeriodicLearner] MBTI analysis disabled, skipping")
            return

        logger.info("[PeriodicLearner] Starting MBTI analysis...")
        ...
        try:
            ...
            if len(recent_logs) < self._mbti_analyzer._min_logs:
                logger.debug(
                    f"[PeriodicLearner] Not enough logs for MBTI analysis "
                    f"({len(recent_logs)} < {self._mbti_analyzer._min_logs})"
                )
                return

            # Get current MBTI profile from store
            mbti_store = getattr(self._memory_system, 'mbti_store', None)
            if not mbti_store:
                logger.debug("[PeriodicLearner] MBTIStore not available")
                return

            current_profile = mbti_store.load_profile()
            ...
            # Run analysis
            result = await self._mbti_analyzer.analyze(
                conversation_logs=log_dicts,
                current_profile=current_profile,
            )

            if result.analysis_skipped or not result.has_adjustments:
                logger.info("[PeriodicLearner] MBTI analysis: no adjustments needed")
                return

            # Apply adjustments to current profile
            dims = (current_profile or {}).get("dimensions", {})
            adjustments = result.dimension_adjustments
            new_dims = {
                "ei": max(0, min(100, dims.get("ei", 50) + adjustments.get("ei", {}).get("delta", 0))),
                "sn": max(0, min(100, dims.get("sn", 50) + adjustments.get("sn", {}).get("delta", 0))),
                "tf": max(0, min(100, dims.get("tf", 50) + adjustments.get("tf", {}).get("delta", 0))),
                "jp": max(0, min(100, dims.get("jp", 50) + adjustments.get("jp", {}).get("delta", 0))),
            }

            # Check cumulative drift from initial YAML values
            initial_dims = self._get_mbti_initial_dims()
            exceeded = self._mbti_analyzer.check_cumulative_drift(new_dims, initial_dims)
            if exceeded:
                logger.warning(
                    f"[PeriodicLearner] MBTI drift exceeded threshold for dimensions: "
                    f"{', '.join(exceeded)} — requiring review, not auto-applying"
                )
                return

            # Build updated profile
            mbti_type = _compute_mbti_type(new_dims)
            updated_profile = {
                "type": mbti_type,
                "dimensions": new_dims,
                "description": result.summary or mbti_store.load_profile().get("description", ""),
                "confidence": max(
                    adj.get("confidence", 0.5)
                    for adj in adjustments.values()
                ),
            }

            # Save via MBTIStore
            mbti_store.save_profile(updated_profile)
            logger.info(
                f"[PeriodicLearner] MBTI analysis applied: type={mbti_type}, "
                f"dims=({new_dims['ei']},{new_dims['sn']},{new_dims['tf']},{new_dims['jp']})"
            )

        except Exception as e:
            logger.warning(f"[PeriodicLearner] MBTI analysis failed: {e}", exc_info=True)

    def _get_mbti_initial_dims(self) -> Dict[str, int]:
        """Get the initial MBTI dimension values from persona config YAML."""
        try:
            persona_config = self._load_persona_config()
            if persona_config:
                mbti = persona_config.get("personality", {}).get("mbti", {})
            ...
            logger.debug(f"[PeriodicLearner] Could not load initial MBTI dims: {e}")
...
def _compute_mbti_type(dimensions: Dict[str, int]) -> str:
    """Compute 4-letter MBTI type label from dimension scores."""
    return (
        ("E" if dimensions.get("ei", 50) > 50 else "I") +
        ("N" if dimensions.get("sn", 50) > 50 else "S") +
        ("T" if dimensions.get("tf", 50) > 50 else "F") +
        ("J" if dimensions.get("jp", 50) > 50 else "P")
    )
