...
│   ├── synthesis/           # 跨源分析 (合成页)
│   │   └── topic.md         # 跨时间线的主题综合
│   └── memes/               # 梗 (周期性学习者生成的幽默/趣味表达)
│       └── meme-xxxx.md     # page_type=meme, 含评分和衰减信息
...
---
title: 页面标题
type: entity | concept | source | synthesis | meme
tags: [tag1, tag2]
...
1. **收集** - 读取所有 entities/ + concepts/ + sources/ + memes/ 页面
2. **分析** - 构建关系图 (双链 + 共现 + 标签)
3. **聚类** - 使用 LLM 识别主题聚类
4. **合并** - 合并重复/相似页面
...
- 路径: `entities/`, `concepts/`, `sources/`, `synthesis/`, `memes/` 五个子目录
